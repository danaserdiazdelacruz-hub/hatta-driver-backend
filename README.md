...
Backend de los módulos Garita, Despacho y Admin. Validado de punta a punta: **25/25 tests de integración** contra Postgres real con las migraciones 001-005 aplicadas (auth, ciclo completo, PIN, rate limit, registro del día).
...
| GET/POST/PATCH | `/api/admin/vehiculos[...]` | admin | CRUD + condición especial (`fn_set_condicion_especial`); placa duplicada → error claro |
| GET/POST/PATCH | `/api/admin/choferes[...]` | admin | CRUD + `POST /:id/vehiculo` → `fn_asignar_chofer_vehiculo` (cierra vinculación previa, requiere migración **007**) |
| GET/POST/PATCH | `/api/admin/rampas[...]` | admin | CRUD + alternar fuera de servicio (una rampa OCUPADA no se toca: se libera sola) |
| GET/POST/PATCH | `/api/admin/perfiles[...]` | admin | Usuarios + `POST /:id/pin` → `fn_asignar_pin` (hash bcrypt) |
...
Nota sobre usuarios: con `AUTH_DISABLED`, `POST /api/admin/perfiles` crea perfiles sueltos (suficiente para asignar PINs de supervisor y probar roles). Con login real, primero se crea el usuario en Supabase Auth y su `id` es el del perfil — en ese momento conviene cambiar el endpoint para usar la Admin API de Supabase (pieza de ~20 líneas).
