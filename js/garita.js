...
/* ============================================================
   ...
   ============================================================ */
/* ---- RASTREO ÚNICO ----
   Una unidad física = UN solo viaje activo, sin excepciones.
   Busca la placa tanto como cabezote como remolque/cola de
   cualquier viaje en curso. Es la primera línea de defensa;
   la definitiva es el constraint UNIQUE en Supabase. */
function unidadDentro(placa) {
  if (!placa) return null;
  return state.viajes.find(x =>
    x.placa === placa ||
    (x.remolque && x.remolque !== 'N/A' && x.remolque === placa)) || null;
}
...
function precheckPlaca(valor) {
  ...
  // ENTRADA
  const dentro = unidadDentro(placa);
  if (dentro) {
    box.className = 'garita-precheck warn';
    box.innerHTML = dentro.placa === placa
      ? `⚠ Ya está dentro · ${esc(dentro.tx || '')} · no se puede duplicar`
      : `⚠ Está dentro como remolque de ${esc(dentro.placa)} · ${esc(dentro.tx || '')}`;
    autoClasif(null);
    return;
  }
  ...
  if (veh || chof) {
    const partes = [];
    if (veh) { if (veh.transportista) partes.push(veh.transportista); if (veh.tipo_vehiculo) partes.push(veh.tipo_vehiculo); if (veh.tipo) partes.push(veh.tipo); }
...
/* La clasificación SIEMPRE está a la vista: qué tipo de camión
   entra es parte esencial de la entrada, nunca un detalle oculto.
   El maestro solo PRE-LLENA los valores y lo marca con el tag
   "✓ del maestro" — el guardia siempre ve y puede corregir. */
function autoClasif(veh, chof) {
  const tag = document.getElementById('gm-maestro-tag');
  if (tag) tag.style.display = veh ? '' : 'none';
  ...
  if (veh && su && veh.tipo) su.value = veh.tipo;
  if (veh && sv && veh.tipo_vehiculo) sv.value = veh.tipo_vehiculo;
  if (chof && ced && !ced.value) ced.value = chof.cedula || '';
}
function clasifCambiada() {
  _clasifManual = true;
  const tag = document.getElementById('gm-maestro-tag');
...
function procesarAcceso(placaRaw, info = {}) {
  ...
  // ENTRADA — rastreo único: la placa no puede estar dentro
  // ni como cabezote ni como remolque de otro viaje activo.
  const dentro = unidadDentro(placa);
  if (dentro) {
    return dentro.placa === placa
      ? scanFeedback('warn', `${placa} ya está dentro`, `Transacción ${dentro.tx || ''}`)
      : scanFeedback('warn', `${placa} está dentro como remolque`, `Unidad ${dentro.placa} · ${dentro.tx || ''}`);
  }
  // La cola que trae tampoco puede estar ya dentro con otra unidad.
  const remolque = (info.remolque || '').toString().toUpperCase().trim();
  if (remolque && remolque !== 'N/A') {
    const colaDentro = unidadDentro(remolque);
    if (colaDentro) return scanFeedback('warn', `Remolque ${remolque} ya está dentro`, `Con ${colaDentro.placa} · ${colaDentro.tx || ''}`);
  }
  ...
  const nuevoViaje = {
    ...
    tipoVehiculo: info.tipoVehiculo || (veh ? veh.tipo_vehiculo : '') || (state.config.catVehiculo[0] || 'Rígido'),
  ...
  api.viajes.insertar(nuevoViaje)
    .then(guardado => {
      // La nube es el árbitro: si otra garita registró esta placa
      // primero, el constraint UNIQUE rechaza el insert. Se anula
      // la entrada local para que no quede un viaje fantasma.
      if (guardado === 'DUPLICADO') {
        const i = state.viajes.findIndex(x => x.id === nuevoViaje.id);
        if (i !== -1) state.viajes.splice(i, 1);
        logAccion('Entrada anulada', `${placa} ya estaba registrada por otra garita · ${tx}`);
        renderAll();
        return scanFeedback('warn', `${placa} ya está dentro`, 'Registrada por otra garita · entrada anulada');
      }
