...
function procesarAcceso(placaRaw, info = {}) {
  ...
  // Insertar en Supabase y añadir a state si tiene éxito
  api.viajes.insertar(nuevoViaje).then(guardado => {
    if (guardado) {
      state.viajes.push(guardado);
    } else {
      state.viajes.push(nuevoViaje); // fallback optimista
    }
