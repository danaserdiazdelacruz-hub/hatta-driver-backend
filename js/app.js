...
/* ============================================================
   TRAMPA GLOBAL DE ERRORES — ningún fallo queda invisible.
   Cualquier excepción o promesa rechazada se muestra en pantalla
   con el mensaje exacto, para poder diagnosticar sin consola.
   ============================================================ */
window.addEventListener('error', (e) => {
  const msg = (e.error && e.error.message) || e.message || 'Error desconocido';
  console.error('[HATTA ERROR]', e.error || e);
  if (typeof showToast === 'function')
    showToast('Fallo del sistema', `${msg} — ${(e.filename || '').split('/').pop()}:${e.lineno || '?'}`, 'danger');
});
window.addEventListener('unhandledrejection', (e) => {
  const msg = (e.reason && e.reason.message) || String(e.reason || 'Promesa rechazada');
  console.error('[HATTA PROMESA]', e.reason);
  if (typeof showToast === 'function')
    showToast('Fallo de sincronización', msg, 'danger');
});
/* ============================================================
   APP — v3.0 OPTIMIZADO
   MEJORAS:
...
function switchTab(tab) {
  // Guardia de acceso: solo módulos asignados al usuario
  if (Array.isArray(state.misModulos) && state.misModulos.length && !state.misModulos.includes(tab)) {
    return showToast('Sin acceso', 'Tu usuario no tiene este módulo asignado', 'warning');
  }
