...
const MAESTRO_SECCIONES = [
  { id: 'rampas',    titulo: 'Rampas',           icono: 'warehouse',       color: '#f59e0b', desc: 'Estado, total y motivos de fuera de servicio',
    cuenta: () => `${state.docksActive.filter(a => a).length}/${numRampas()} activas` },
  { id: 'catalogos', titulo: 'Catálogos',        icono: 'database',        color: '#22c55e', desc: 'Vehículos, choferes, transportistas, destinos y motivos',
    cuenta: () => `${CAT.tipos().reduce((n, t) => n + CAT.items(t).length, 0)} registros` },
  { id: 'usuarios',  titulo: 'Usuarios',         icono: 'users',           color: '#3b82f6', desc: 'Administrador de acceso: qué módulo ve cada quien',
    cuenta: () => `${state.users.filter(u => u.activo !== false).length} activos` },
  { id: 'flujo',     titulo: 'Flujo y tiempos',  icono: 'git-pull-request', color: '#a78bfa', desc: 'SLA por etapa, modo de salida y reglas de patio',
    cuenta: () => '' },
  { id: 'bitacora',  titulo: 'Bitácora',         icono: 'scroll-text',     color: '#2dd4bf', desc: 'Auditoría de todo lo que pasa en el sistema',
    cuenta: () => `${state.bitacora.length} eventos` },
  { id: 'sistema',   titulo: 'Sistema',          icono: 'server-cog',      color: '#ef4444', desc: 'Datos de prueba y zona de peligro',
    cuenta: () => '' },
];
...
function renderMaestroHub() {
  ...
  hub.innerHTML = MAESTRO_SECCIONES.map(sec => `
    <button class="hub-card" style="--secc:${sec.color}" onclick="setMaestroSeccion('${sec.id}')">
