...
function _sortVal(k, h) {
  switch (k) {
    ...
    case 'fecha': return new Date(h.entrada || 0).getTime();
...
function renderHistorial() {
  ...
  if (isMobile) {
    ...
    tb.innerHTML = pageData.map(h => {
      const resp = `Garita: ${esc(h.garitaEntradaPor || '—')}${h.garitaSalidaPor ? ' / ' + esc(h.garitaSalidaPor) : ''} · Tráfico: ${esc(h.traficoPor || '—')} · Despacho: ${esc(h.despachoPor || '—')} · Rampa: ${(h.rampaUsada ?? h.dock) != null ? (h.rampaUsada ?? h.dock) : '—'}`;
      return `<tr title="${resp}">
        <td class="mono fecha-cell">${_dateDisplay(h.entrada)}</td>
        ...
        <td class="mono num">${formatHora(h.entrada)}</td>
        <td class="mono num">${h.salida && _dateDisplay(h.salida) !== _dateDisplay(h.entrada) ? `<span class="hist-fecha">${_dateDisplay(h.salida)}</span>` : ''}${h.salida ? formatHora(h.salida) : '—'}${h.garitaSalidaPor ? `<span class="hist-by">por ${esc(h.garitaSalidaPor)}</span>` : ''}</td>
        <td class="mono num">${(h.rampaUsada ?? h.dock) != null ? 'R-' + (h.rampaUsada ?? h.dock) : '—'}</td>
        <td>${esc(h.destino || '—')}</td>
...
/* ---- Desplazamiento lateral del historial (tabla ancha) ---- */
function histScrollX(dir) {
  const box = document.querySelector('#tab-historial .hist-scroll');
  if (!box) return;
  box.scrollBy({ left: dir * Math.round(box.clientWidth * 0.7), behavior: 'smooth' });
}
