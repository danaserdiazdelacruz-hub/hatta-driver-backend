...
function setNumRampas(valor) {
  ...
  api.config.guardar(state.config).catch(() => {});
...
function _addCat(e, arr) { e.preventDefault(); const v = (e.target.v.value || '').trim(); if (v && !arr.includes(v)) arr.push(v); e.target.reset(); api.config.guardar(state.config).catch(() => {}); renderClasifConfig(); renderAll(); }
function addCatUnidad(e) { _addCat(e, state.config.catUnidad); }
function addCatVehiculo(e) { _addCat(e, state.config.catVehiculo); }
function delCatUnidad(i) { if (state.config.catUnidad.length <= 1) return showToast('Atención', 'Deja al menos un valor', 'warning'); state.config.catUnidad.splice(i, 1); api.config.guardar(state.config).catch(() => {}); renderClasifConfig(); renderAll(); }
function delCatVehiculo(i) { if (state.config.catVehiculo.length <= 1) return showToast('Atención', 'Deja al menos un valor', 'warning'); state.config.catVehiculo.splice(i, 1); api.config.guardar(state.config).catch(() => {}); renderClasifConfig(); renderAll(); }
function toggleSelPatio(k, val) { state.config.selPatio[k] = !!val; api.config.guardar(state.config).catch(() => {}); renderClasifConfig(); renderAll(); }
