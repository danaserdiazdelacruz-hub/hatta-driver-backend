...
function resetSystem() {
  uiConfirm(
    ...
    'Se eliminarán TODOS los viajes, historial, rechazos y demoras de esta empresa, en este equipo y en la nube. Los catálogos, usuarios, configuración y la bitácora de auditoría se conservan. Esta acción no se puede deshacer.',
    () => { _resetDatosOperativos(); },
    { danger: true, okText: 'Borrar todo' }
  );
}

function _resetDatosOperativos() {
  // 1) Local primero: la UI queda limpia YA
  state.viajes  = [];
  state.history = [];
  state.rechazos = [];
  state.demoras  = [];
  state.docksMotivo = {};
  state.docksActive = Array(numRampas()).fill(true);
  logAccion('Borrado de datos operativos', 'Viajes, historial, rechazos y demoras eliminados');
  renderAll();
  showToast('Datos operativos borrados', 'Sincronizando con la nube…', 'success');
  // 2) Nube detrás, con error visible
  api.reset().then(ok => {
    if (!ok) showToast('Borrado SOLO en este equipo', 'La nube no confirmó el borrado', 'danger');
  }).catch(err => showToast('Borrado SOLO en este equipo', err.message || String(err), 'danger'));
}
