...
const api = {
  ...
  /* Preferencias de UI locales al equipo (no van a Supabase).
     showAppForRole lee esta misma clave al iniciar sesión. */
  ui: {
    guardarTab(tab) {
      try { localStorage.setItem(HATTA.LS_PREFIX + 'tab', JSON.stringify(tab)); } catch {}
    },
  },
  ...
  auth: {
    async login(email, password) {
      ...
      const { data: perfil, error: pe } = await _sb
        .from('usuarios')
        .select('id, empresa_id, nombre, rol, activo, modulos')
      ...
      _session = {
        ...
        rol: perfil.rol, email: data.user.email, modulos: perfil.modulos,
      };
    ...
    async restaurarSesion() {
      ...
      const { data: perfil } = await _sb
        .from('usuarios').select('id, empresa_id, nombre, rol, activo, modulos')
  ...
  viajes: {
    ...
    async insertar(v) {
      ...
      if (error) {
        // 23505 = violación de unicidad: otra garita registró esta placa
        // (o su remolque) primero. No es un fallo del sistema — el
        // llamador decide qué hacer (garita anula la entrada local).
        if (error.code === '23505') return 'DUPLICADO';
        api._err('viajes.insertar', error); return null;
      }
  ...
  usuarios: {
    async leer() {
      ...
      const { data, error } = await _sb.from('usuarios')
        .select('id, nombre, rol, activo, ultimo_login, modulos').eq('empresa_id', eid);
    ...
    async actualizar(id, campos) {
      const permitidos = {};
      ['nombre', 'rol', 'modulos', 'activo'].forEach(k => { if (k in campos) permitidos[k] = campos[k]; });
      const { error } = await _sb.from('usuarios').update(permitidos).eq('id', id);
      if (error) { api._err('usuarios.actualizar', error); return false; }
      return true;
    },
  },
  ...
  /* Borrado REAL de datos operativos de la empresa (viajes, historial,
     rechazos, demoras). NO toca: catálogos, config, usuarios, bitácora
     (la auditoría no se borra — principio OEA/BASC). */
  reset: async () => {
    const eid = api._eid(); if (!eid) return false;
    const tablas = ['viajes', 'historial', 'rechazos', 'demoras'];
    const res = await Promise.all(tablas.map(t =>
      _sb.from(t).delete().eq('empresa_id', eid)
    ));
    const fallo = res.find(r => r.error);
    if (fallo) { api._err('reset', fallo.error); return false; }
    return true;
  },
};
