...
describe("admin: maestros", () => {
  ...
  it("crea rampa, duplicada falla, y fuera de servicio alterna", async () => {
    ...
    const fs = (await call("PATCH", `/api/admin/rampas/${r.id}`, { fuera_servicio: true, motivo: "Averiada - puerta dañada" })).json();
    expect(fs.success).toBe(true);
    const estado = await q<{ estado: string; motivo_fuera_servicio: string | null }>(
      `select estado, motivo_fuera_servicio from rampas where id=$1`, [r.id]);
    expect(estado[0].estado).toBe("fuera_servicio");
    expect(estado[0].motivo_fuera_servicio).toBe("Averiada - puerta dañada");

    // Al habilitar, el motivo se limpia solo
    await call("PATCH", `/api/admin/rampas/${r.id}`, { fuera_servicio: false });
    const limpio = await q<{ estado: string; motivo_fuera_servicio: string | null }>(
      `select estado, motivo_fuera_servicio from rampas where id=$1`, [r.id]);
    expect(limpio[0].estado).toBe("disponible");
    expect(limpio[0].motivo_fuera_servicio).toBeNull();
  });
