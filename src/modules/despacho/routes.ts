...
export async function despachoRoutes(app: FastifyInstance) {
  ...
  app.get("/api/despacho/contexto", auth, async (req, reply) => {
    const [rampas, unidades] = await Promise.all([
      ...
      q(
        // Cola del patio: urgentes primero, luego refrigerados, luego espera
        `select v.id, v.placa, v.tipo_unidad, v.condicion_especial,
                coalesce(vi.prioridad, 'normal') as prioridad,
                vi.destino,
          ...
          order by case coalesce(vi.prioridad, 'normal')
                     when 'urgente' then 0 when 'refrigerado' then 1 else 2 end,
                   vi.abierto_en nulls last`,
        [req.usuario.cd_id]
      ),
    ]);
